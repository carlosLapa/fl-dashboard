...
export const updateTarefaStatusAPI = async (
  ...
  try {
    console.log(
      `[API] Iniciando atualização de status da tarefa ${tarefaId} para "${newStatus}"`
    );
    ...
    console.log(
      `[API] Resposta recebida em ${endTime - startTime}ms - Status: ${
        response.status
      }`
    );
    ...
    if (response.data) {
      console.log(
        `[API] Tarefa atualizada - ID: ${response.data.id}, Status: ${response.data.status}`
      );
      ...
      if (response.data.projeto) {
        console.log(
          `[API] Projeto associado - ID: ${response.data.projeto.id}, Nome: "${response.data.projeto.designacao}"`
        );
      } else {
    ...
    console.error(
      `[API] ERRO ao atualizar status da tarefa ${tarefaId}:`,
      error
    );
    ...
    if (axios.isAxiosError(error)) {
      if (error.response) {
        ...
        console.error(
          `[API] Dados do erro: ${JSON.stringify(error.response.data)}`
        );
      } else if (error.request) {
        console.error('[API] Requisição feita mas sem resposta');
      } else {
        console.error(
          `[API] Erro na configuração da requisição: ${error.message}`
        );
      }
    }
