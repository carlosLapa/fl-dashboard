...
const EditExternoModal: React.FC<EditExternoModalProps> = ({
  ...
  const [formData, setFormData] = useState<ExternoUpdateDTO>({
    id: 0,
    name: '',
    email: '',
    telemovel: '',
  ...
  useEffect(() => {
    if (externo) {
      setFormData({
        id: externo.id, // Make sure this is included
        name: externo.name,
        email: externo.email,
        telemovel: externo.telemovel,
        preco: externo.preco,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const savedExterno = await updateExternoAPI(externo.id, updateData);
  ...
  const selectedEspecialidadesOptions: EspecialidadeOption[] =
    formData.especialidades.map((esp) => ({
      value: esp,
      label:
        especialidadesOptions.find((option) => option.value === esp)?.label ||
        esp,
    }));
  ...
  return (
    <Modal show={show} onHide={onHide}>
      <Modal.Header closeButton>
        <Modal.Title>Editar Colaborador Externo</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Form>
          ...
          <Form.Group controlId="formEspecialidades" className="mb-3">
            ...
            <Select<EspecialidadeOption, true>
              ...
              value={selectedEspecialidadesOptions}
